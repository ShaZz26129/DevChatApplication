...
public partial class GroupChatMessageViewModel : BaseViewModel, IQueryAttributable
{
    ...
    public GroupChatMessageViewModel(CommonChatModel chatModel)
    {
        ...
        // Ensure hubConnection is initialized once
        if (hubConnection == null)
        {
            hubConnection = ChatHelper.GetInstanse(chatUserName);
            ConfigureHubConnection(chatUserName);
        }
        //hubConnection = ChatHelper.GetInstanse(chatUserName);
    ...
    private void ConfigureHubConnection(string chatUserName)
    {
        // Check for null to avoid adding handlers multiple times
        if (hubConnection != null)
        {
            ...
            hubConnection.On<string, string, string, bool>("ReceiveMessage", (user, profilePic, message, isMe) =>
            {
                if (user != MyUserId)
                {
                    ChatMessageList.Add(new ChatMessage() {Message = message, UserName = user, MyPhoto = profilePic, IsOwnMessage = isMe, IsSystemMessage = false, ActionTime = DateTime.Now.ToString("hh:mm tt") });
                }
                //ChatMessageList.Add(new ChatMessage() { Message = message, IsOwnMessage = isMe, MyPhoto = this.MyPhoto, IsSystemMessage = false, ActionTime = DateTime.Now.ToString("hh:mm tt") });
            });
    ...
    //}
    //[RelayCommand]
    //async private void SendMessageToGroupChat()
    //{
    //    try
    //    {
